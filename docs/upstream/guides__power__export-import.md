...
Replace \`\` with your company's ID (visible in the URL when you're viewing the company in Paperclip).
...
paperclipai company export  --out ./full-export \
...
\| Option \| What it does \|
\|---\|---\|
\| \`--out \` \| Where to save the export (required) \|
\| \`--include \` \| What to include: \`company\`, \`agents\`, \`projects\`, \`issues\`, \`tasks\`, \`skills\` — comma-separated \|
\| \`--skills \` \| Export only specific skills by name \|
\| \`--projects \` \| Export only specific projects \|
...
paperclipai company export  --out ./engineering-team --include agents
