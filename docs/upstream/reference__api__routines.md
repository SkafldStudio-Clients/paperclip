...
\| \`concurrencyPolicy\` \| no \| Defaults to \`coalesce\_if\_active\`. \|
\| \`catchUpPolicy\` \| no \| Defaults to \`skip\_missed\`. \|
...
curl -X POST "http://localhost:3100/api/companies/company-1/routines" \
 ...
 -d '{
 "title": "Weekly CEO briefing for {{company\_name}}",
 ...
 "priority": "medium",
 "status": "active",
 "concurrencyPolicy": "coalesce\_if\_active",
 "catchUpPolicy": "skip\_missed",
 "variables": \[\
 {\
 "name": "company\_name",\
 "label": "Company name",\
...
response = requests.post(
 "http://localhost:3100/api/companies/company-1/routines",
 headers={
 "Authorization": f"Bearer {board\_token}",
 "Content-Type": "application/json",
 },
 json={
 "title": "Weekly CEO briefing for {{company\_name}}",
...
\- \`hmac\_sha256\` uses \`X-Paperclip-Signature\` or \`X-Hub-Signature-256\`
\- \`github\_hmac\` uses \`X-Hub-Signature-256\` or \`X-Paperclip-Signature\` plus \`X-Paperclip-Timestamp\`
...
\`\`\`bash
curl -X POST "http://localhost:3100/api/routine-triggers/public//fire" \
 -H "Authorization: Bearer " \
 -H "Content-Type: application/json" \
 -d '{
 "payload": {
...
\`\`\`javascript
const res = await fetch("/api/routine-triggers/public//fire", {
 method: "POST",
 headers: {
...
response = requests.post(
 "http://localhost:3100/api/routine-triggers/public//fire",
 headers={
 "Authorization": f"Bearer {webhook\_secret}",
 "Content-Type": "application/json",
 },
 json={
 "payload": {
...
\| \`issue\_created\` \| A new execution issue was created. \|
...
\- If a run finds an active live execution issue and the concurrency policy is not \`always\_enqueue\`, the run is linked or skipped instead of creating new work.
