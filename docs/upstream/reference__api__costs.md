...
\- \`billingType\` if you want to distinguish \`metered\_api\`, \`subscription\_included\`, \`subscription\_overage\`, \`credits\`, \`fixed\`, or \`unknown\`
...
\- \`metric\` defaults to \`billed\_cents\`
\- \`windowKind\` defaults to \`calendar\_month\_utc\` for company and agent scopes
...
Useful event kinds include things like \`inference\_charge\`, \`platform\_fee\`, \`credit\_purchase\`, \`credit\_refund\`, and \`manual\_adjustment\`. The API also stores \`direction\`, \`amountCents\`, \`currency\`, \`estimated\`, and optional metadata.
