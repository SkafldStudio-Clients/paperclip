...
\> \*\*Note:\*\* The app logic does not change across modes. Only \`DATABASE\_URL\` and the storage location change.

\-\-\-

\## Embedded PostgreSQL

This is the default mode when \`DATABASE\_URL\` is not set.
...
\`\`\`sh
cp .env.example .env
\# DATABASE\_URL=postgres://paperclip:paperclip@localhost:5432/paperclip
\`\`\`
...
3\. set \`DATABASE\_URL\` in your deployment environment
