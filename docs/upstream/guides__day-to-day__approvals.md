...
1\. \*\*Open the Approvals page\*\*
 ...
 Pending approvals have a yellow badge. Items in \`revision\_requested\` are still treated as actionable until the requester follows up and resubmits.
...
\- \*\*Pending\*\* — everything that still needs a decision from you. This includes approvals in the \`pending\` state and any in \`revision\_requested\` — the latter are approvals you previously sent back, still awaiting the requester's follow-up. Both count toward the yellow pill beside the tab label.
...
Every actionable approval (anything in \`pending\` or \`revision\_requested\`) shows the same three buttons at the bottom of the detail card. Use them deliberately — each one means something different to the requesting agent.
...
\- The approval enters "revision\_requested" state.
...
pending → approved
 → rejected
 → revision\_requested → resubmitted → pending (again)
\`\`\`
