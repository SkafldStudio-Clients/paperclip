...
\- An API key from Anthropic (for the \`claude\_local\` adapter) or OpenAI (for \`codex\_local\`) — see the \[Installation guide\](./installation.md) for how to get one
\- \*\*For \`claude\_local\`:\*\* \[Claude Code\](https://docs.anthropic.com/en/docs/claude-code) installed on your Mac
...
3\. \*\*Choose an adapter\*\*
 ...
 \- \*\*Environment variables\*\* — Add \`ANTHROPIC\_API\_KEY\` and either paste the key as a plain value or store it as a Paperclip secret. This is how the adapter gets access to Claude.

 \- \*\*Test environment\*\* — Use this button to confirm Paperclip can see Claude Code and that your \`ANTHROPIC\_API\_KEY\` binding works before you create the agent.
 ...
 \*\*Codex Local\*\* runs an OpenAI Codex agent directly on your Mac. Like \`claude\_local\`, it has access to your filesystem and can run commands within its working directory — but it's powered by OpenAI's models rather than Anthropic's.
 ...
 \- \*\*Environment variables\*\* — Add \`OPENAI\_API\_KEY\` and either paste the key as a plain value or store it as a Paperclip secret.
...
7\. \*\*Test the environment again if needed\*\*
 ...
 Paperclip will attempt to connect to the adapter — in the case of \`claude\_local\`, it will check that Claude Code is installed and accessible, and that the environment variable binding is valid.
...
\- The environment variable binding is wrong — double-check that you've set \`ANTHROPIC\_API\_KEY\` or \`OPENAI\_API\_KEY\`, and that the value itself is correct with no extra spaces.
