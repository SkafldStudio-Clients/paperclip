...
export function createStdoutParser() {
 ...
 awaitingResult = true;
 return \[{ kind: "tool\_call", ts, name: "shell", input: {}, toolUseId }\];
 }

 if (awaitingResult) {
 awaitingResult = false;
 return \[{ kind: "tool\_result", ts, toolUseId: \`tool-${counter}\`, content: trimmed, isError: false }\];
 }
...
{ kind: "tool\_call"; ts: string; name: string; input: unknown; toolUseId?: string }
{ kind: "tool\_result"; ts: string; toolUseId: string; content: string; isError: boolean }
