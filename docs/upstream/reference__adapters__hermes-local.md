...
\`hermes\_local\` runs \[Hermes Agent\](https://github.com/NousResearch/hermes-agent) — a full-featured AI agent by Nous Research — on the same machine as Paperclip. Use it when you want persistent memory, a 30+ tool suite, 80+ loadable skills, multi-provider model routing, and MCP client support in a single adapter.
...
\| Field \| Required \| Default \| Notes \|
\|---\|---:\|---\|---\|
\| \`toolsets\` \| no \| all \| Comma-separated list of toolsets. Available: \`terminal\`, \`file\`, \`web\`, \`browser\`, \`code\_execution\`, \`vision\`, \`mcp\`, \`creative\`, \`productivity\`. \|
