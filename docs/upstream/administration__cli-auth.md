...
Board claim only exists when \_all\_ of these are true:
...
\`\`\`
http://localhost:3000/board-claim/?code=```
```
...
paperclipai auth login --company-id
...
Once logged in, the CLI sends `Authorization: Bearer ` on every request. You can verify by running:
