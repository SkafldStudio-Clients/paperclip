...
\- A Paperclip Cloud stack URL that advertises the \`cloud\_sync\` transfer feature flag at \`/.well-known/paperclip-upstream\`.
\- The local \`companyId\` you want to push.
\- Enough rights on the cloud stack to approve the authorization grant — the CLI requests the scopes \`upstream\_import:preview\`, \`upstream\_import:write\`, and \`upstream\_import:read\`.
...
\`\`\`bash
curl -X PATCH "$PAPERCLIP\_API\_URL/api/instance/settings/experimental" \
...
The CLI generates a fresh source identity (an ed25519 keypair plus a fingerprint), opens a browser to the cloud stack's PKCE authorize URL, and stores the resulting token under \`~/.paperclip//secrets/cloud-upstream-connections.json\`. The file is written with mode \`0600\` — keep it that way.
...
\`\`\`bash
paperclipai cloud push --company "$LOCAL\_COMPANY\_ID"
\`\`\`
...
\`\`\`bash
curl -X POST \
 "$PAPERCLIP\_API\_URL/api/cloud-upstreams/$CONNECTION\_ID/push-runs/$RUN\_ID/activation" \
 -H "Authorization: Bearer $PAPERCLIP\_API\_KEY" \
 -H "Content-Type: application/json" \
 -d '{ "companyId": "'$LOCAL\_COMPANY\_ID'", "entityType": "agents" }'
\`\`\`
...
\`\`\`bash
curl -sS "$PAPERCLIP\_API\_URL/api/cloud-upstreams?companyId=$LOCAL\_COMPANY\_ID" \
 -H "Authorization: Bearer $PAPERCLIP\_API\_KEY"
\`\`\`
...
\| \`Remote Paperclip Cloud stack does not advertise the cloud\_sync transfer flag.\` \| The cloud stack is reachable but doesn't enable cloud sync. Ask the stack admin to advertise the \`cloud\_sync\` feature flag. \|
