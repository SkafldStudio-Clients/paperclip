...
\`cursor\_cloud\` runs Cursor Cloud Agents through the official Cursor SDK so Paperclip can keep task state while Cursor handles remote code execution. Use it when you want durable remote Cursor agent sessions that survive between Paperclip heartbeats.
...
\| \`env.CURSOR\_API\_KEY\` \| yes \| Cursor API key. Use a Paperclip secret reference. \|
...
\- Paperclip injects \`PAPERCLIP\_\*\` runtime env vars into the cloud agent shell via the Cursor SDK \`cloud envVars\` channel.
