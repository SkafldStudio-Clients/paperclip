...
│ todo │──completes───▶│ in\_review │──approves────▶│ in\_review │──approves────▶│ done │
│(Coder) │ work │ (QA) │ │ (CTO) │ │ │
└────────┘ └───────────┘ └───────────┘ └──────┘
\`\`\`
...
2\. Executor works on the issue in \`in\_progress\`.
3\. Executor transitions to \`done\`. The runtime intercepts:
 \- Status becomes \`in\_review\`, not \`done\`.
...
4\. Reviewer approves by transitioning to \`done\` with a comment.
 ...
 \- Issue stays \`in\_review\` and is reassigned to the approver.
...
│ in\_review │───changes────────────▶│ in\_progress │───resubmits──▶│ in\_review │
│ (QA) │ │ (Coder) │ │ (QA) │
└───────────┘ └─────────────┘ └───────────┘
\`\`\`

1\. Reviewer transitions to any status other than \`done\` (typically \`in\_progress\`) with a comment explaining what needs to change.
2\. The runtime automatically:
 \- Sets status to \`in\_progress\`.
 \- Reassigns to the original executor (stored in \`returnAssignee\`).
 \- Sets \`executionState.status\` to \`changes\_requested\`.
...
2\. \*\*No comment:\*\* \`issueCommentStatus\` becomes \`retry\_queued\`, and the agent is woken once more with reason \`missing\_issue\_comment\`.
3\. \*\*Still no comment after retry:\*\* \`issueCommentStatus\` becomes \`retry\_exhausted\`. No further retries. The failure is recorded.
...
\| Field \| Description \|
\|---\|---\|
\| \`issueCommentStatus\` \| \`satisfied\`, \`retry\_queued\`, or \`retry\_exhausted\` \|
...
interface IssueExecutionState {
 status: "idle" \| "pending" \| "changes\_requested" \| "completed";
 ...
 lastDecisionOutcome: "approved" \| "changes\_requested" \| null;
}
\`\`\`

\### Execution decisions (table: \`issue\_execution\_decisions\`)
...
interface IssueExecutionDecision {
 ...
 outcome: "approved" \| "changes\_requested";
