...
When you run \`paperclipai plugin install \`, the CLI does two things differently from a \`paperclipai plugin install @scope/name\` install:
...
\| Flag \| What it does \|
\|---\|---\|
\| \`--output \` \| Parent directory to create the plugin folder in. Default is the current directory. \|
\| \`--template \` \| Starter shape. \`default\` is a good first plugin; \`connector\` adds outbound-integration scaffolding; \`workspace\` adds project-scoped surfaces; \`environment\` scaffolds an environment-driver plugin. \|
\| \`--category \` \| Manifest category — the chip that shows up on the Plugin Manager page. \|
\| \`--display-name \` \| Manifest display name. \|
\| \`--description \` \| Manifest description. \|
\| \`--author \` \| Manifest author. \|
\| \`--sdk-path \` \| Absolute path to a specific \`packages/plugins/sdk\` checkout. Useful if you have more than one Paperclip clone. \|
...
\`node\_modules\`, \`.git\`, \`.paperclip-sdk\`, and other dotfolders are ignored by the watcher. Adding a new dependency requires actually importing it from your code and letting the watch build rebuild before the worker sees it.
...
Each row includes a ready-to-run \`paperclipai plugin install \` line.
